...
app.get('/meta_info', cors(), (req, res) => {
  ...
  let out = [];
  request.getResponse().then(collage_info => {
...
app.get('/album_info', async (req, res) => {
  let headers = req.headers;
  fetchData(headers.artist_url).catch(err => Promise.reject(err).then(console.log(err)).then(res.send(err))).then(result => res.send(result));
});
